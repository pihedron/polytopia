...
import Unit, { Archer, Warrior } from './Unit.js'
    ...
    this.mapTiles((tile, x, z) => {
      ...
      switch (tile.terrain) {
        case 'land': {
          ...
          if (tile.unit) {
            const height = 1
            const color = tile.unit.team === 0 ? 0x0000ff : 0xff0000
            const unit = new THREE.Mesh(new THREE.BoxGeometry(0.5, height, 0.5), new THREE.MeshPhongMaterial({ color }))
      ...
      if (this.indicators.length > 0) {
        this.indicators.forEach(indicator => {
          if (indicator.position.x + this.offset === this.pointer.x && indicator.position.z + this.offset === this.pointer.z) {
            ...
            /** the defending unit */
            let unit = this.board[this.pointer.x][this.pointer.z].unit

            if (unit) {
              const { attackResult, defenseResult} = this.tile.unit?.fight(unit, 1)
              unit.health -= attackResult
              if (unit.health > 0 && Math.max(Math.abs(unit.x - this.tile.unit.x), Math.abs(unit.z - this.tile.unit.z)) <= unit.range) {
                this.tile.unit.health -= defenseResult
              }
            }
      ...
      if (this.tile.unit) {
        ...
        tiles.forEach(tile => {
          ...
          indicator.add(
            new THREE.Mesh(
              new THREE.RingGeometry(1 / 4, 1 / 2),
              new THREE.MeshBasicMaterial({ color: tile.unit ? 0xff0000 : 0x0000ff, opacity: 1 / 2, transparent: true }),
            )
          )
